...
void buildColliders(Dot* dot)
{
    ...
    Dot_createColliders(dot, nRects, &rects[0]);
}
...
int main( int argc, char* args[] )
{
	...
	else
	{
		...
		else
		{
			...
			Dot otherDot = Dot_create( gDotTexture, SCREEN_WIDTH, SCREEN_HEIGHT );
			otherDot.mPosX = SCREEN_WIDTH / 4;
			otherDot.mPosY = SCREEN_HEIGHT / 4;
			buildColliders(&otherDot);
			...
			while( !quit )
			{
				...
				Dot_render(otherDot, gRenderer);

				//Update screen
				SDL_RenderPresent( gRenderer );
			}
			Dot_clearColliders(&dot);
			Dot_clearColliders(&otherDot);
		}
	}
